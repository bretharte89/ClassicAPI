...
//   }
//
// Synchronous *peek*: returns the data only if the creature is already
// cached (seen this session, or loaded from creaturecache.wdb at login);
// returns nil otherwise. To fetch an uncached creature, the companion
// `C_CreatureInfo.RequestLoadCreatureByID` issues the query and fires a
// `CREATURE_DATA_LOAD_RESULT` event when it lands (see — to be added).
//
