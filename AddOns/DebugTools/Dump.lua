------------------------------------------------------------------------------
-- Dump.lua  (1.12 / Lua 5.0 backport)
--
-- Original by Iriel, Esamynn and Kirov from DevTools v1.11
-- /dump implementation
--
-- Globals: DevTools, SLASH_DUMP1, DevTools_Dump, DevTools_RunDump
...
-- Grab a copy of various oft-used functions
local rawget = rawget;
local type = type;
...
local function prepSimpleKey(val, context)
    ...
    if (valType == "string") then
        ...
        if ((l <= DEVTOOLS_LONG_STRING_CUTOFF) or
            (DEVTOOLS_LONG_STRING_CUTOFF <= 0)) then
            if (string_find(val, "^[a-zA-Z_][a-zA-Z0-9_]*$")) then
                return val;
            end
        end
    end
...
-- Strict weak order across mixed key types so table.sort is well-defined:
-- numbers first (by value), then strings (by value), then everything else
-- (by tostring). Booleans/tables/etc. fall through to tostring comparison.
local function _DumpKeyLess(a, b)
...
local function DevTools_DumpTableContents(val, prefix, firstPrefix, context)
    ...
    -- Collect and sort keys so dump output is deterministic. pairs() walks
    -- the hash part in internal order, which surfaces mixed-key tables out
    -- of numeric order (e.g. GetFactionInfo's packed return values).
    local keys = {};
    for k in pairs(val) do
        table.insert(keys, k);
    end
    table.sort(keys, _DumpKeyLess);
    ...
    for i = 1, total do
        ...
        if ((showCount <= DEVTOOLS_MAX_ENTRY_CUTOFF) or
            ...
            DevTools_DumpValue(v, prefix, rp, trailing, context);
        end
    end
...
function DevTools_DumpValue(val, prefix, firstPrefix, suffix, context)
    ...
    firstPrefix = prefix;
    DevTools_DumpTableContents(val, prefix, firstPrefix, context);
...
-- Dump the specified list of values
...
-- Slash command setup
SLASH_DUMP1 = "/dump";
SlashCmdList["DUMP"] = DevTools_DumpCommand;
